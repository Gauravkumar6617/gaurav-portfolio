...
export async function GET() {
  const rows = await sql`
    SELECT id, title, category, description, link, image_url, featured, sort_order, created_at
    FROM projects
...
export async function POST(request: NextRequest) {
  ...
  const rows = await sql`
    INSERT INTO projects (title, category, description, link, image_url, featured, sort_order)
    VALUES (${title}, ${category}, ${description}, ${link}, ${image_url}, ${featured}, ${sort_order})
